...
def _train_in_container() -> None:
    ...
    Selects between DDP and FSDP based on the `--fsdp` flag:
      ...
      - DDP (default): each rank holds the full model. Used for 8B / 7B
        models. Post-training merges the adapter off the in-memory model.
      - FSDP (`--fsdp true`): shards the base across ranks. Used for 32B /
        70B models that don't fit per-GPU. Requires SageMaker to launch via
        `distribution={"torch_distributed": {"enabled": True}}`. Post-training
        rank 0 reloads the base on CPU, applies the adapter, merges, pushes.

    The merged standalone model is the only artifact pushed to the Hub.
    """
    import gc
    import shutil
    ...
    # Train loss on the assistant turn only. Without this, the model memorises
    # the system + user prompt and reproduces it at inference. Prefer
    # completion_only_loss (works with any chat template by detecting the
    # prompt/completion split) over assistant_only_loss (requires the chat
    # template to contain {% generation %} markers, which OLMo-2 / Llama-3
    # stock templates lack).
    ...
    # Required when gradient checkpointing is on (use_cache is incompatible).
